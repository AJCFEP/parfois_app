...
# -------------------------------------------------
# Recommendation files – text / image / hybrid
# (these CSVs are generated by the notebook)
# -------------------------------------------------
...
def load_data():
    ...
    rec_text   = pd.read_csv(REC_TEXT_CSV)
    rec_image  = pd.read_csv(REC_IMAGE_CSV)
    rec_hybrid = pd.read_csv(REC_HYBRID_CSV)
    ...
    return df_products, df_sales, rec_dfs
...
df_products, df_sales, rec_dfs = load_data()
...
sim_mode = st.radio(
    ...
    index=0,          # default = Text only (original behaviour)
    horizontal=True,  # buttons side by side
)
